...
(function($){
	...
	$.fn.rssfeed = function(url, options, fn) {	
		...
		var defaults = {
			...
			sortasc: true,
			historical: false,
			noTitleLink: false // added by TH to accommodate project RSS feeds that include the whole content
		};  
	...
	var _process = function(e, data, options) {
		...
		for (var i=options.offset; i<feeds.entries.length; i++) {
			...
			// TH added the conditional to remove the title link
			if (options.noTitleLink) {
			rowArray[rowIndex]['html'] = '<'+ options.titletag +'>' + entry.title +'</'+ options.titletag +'>';
			} else {
			...
			if (options.date && pubDate) rowArray[rowIndex]['html'] += '<div class="rssPubDate">'+ pubDate +'</div>'
			if (options.content) {
				...
				rowArray[rowIndex]['html'] += content;
			}
